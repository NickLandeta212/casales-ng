...
export class PagoMultaComponent implements OnInit {
  ...
  get multasDepartamento(): Multa[] {
    ...
    return this.multas.filter((multa) => multa.departamentoId.toString() === departamentoValue.toString());
  }

  get totalCancelar(): number {
    return this.multasSeleccionadas.reduce((total, multa) => total + Number(multa.monto || 0), 0);
  }

  get multasSeleccionadas(): Multa[] {
    return this.multasDepartamento.filter((multa) => this.isMultaSeleccionada(multa));
  }
  ...
  enviarPago(): void {
    ...
    if (this.pagoForm.invalid || !this.comprobanteBase64 || this.multasSeleccionadas.length === 0) {
      this.pagoForm.markAllAsTouched();

      if (this.multasDepartamento.length === 0 && this.pagoForm.get('departamento')?.value) {
        this.errorMessage = 'No hay multas registradas para el departamento seleccionado.';
      }

      if (this.multasDepartamento.length > 0 && this.multasSeleccionadas.length === 0) {
    ...
    this.multasService.registrarPagoMultas({
      ...
      next: () => {
        ...
        this.successMessage = 'Pago enviado correctamente. Queda pendiente de revision.';
        this.pagoForm.reset();
        this.limpiarSeleccionMultas();
  ...
  private getMultaKey(multa: Multa): string {
    return multa.id !== undefined && multa.id !== null ? multa.id.toString() : '';
  }
